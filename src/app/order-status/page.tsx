  ...
  if (order === undefined) {
    return (
      <div className="min-h-screen pt-[calc(var(--header-height)+24px)] px-4">
        <div className="max-w-2xl mx-auto py-20 text-center text-sm text-(--color-text-muted)">
          Loading your order…
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-(--color-bg) pt-[calc(var(--header-height)+24px)] pb-16 px-4 sm:px-6">
      <div className="max-w-2xl mx-auto">
        ...
        >
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold text-(--color-text-strong)">
            Order placed!
          </h1>
        ...
        <div className="rounded-lg border border-(--color-border) bg-(--color-surface) p-5 mb-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">Order ID</p>
              <p className="mt-1 font-mono text-sm text-(--color-text-strong)">{order.id}</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">Total paid</p>
              <p className="mt-1 text-sm font-semibold tabular text-(--color-text-strong)">
                {formatPrice(order.totals.totalINR)}
              </p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">Payment</p>
              <p className="mt-1 text-sm text-(--color-text-strong)">{PAYMENT_LABEL[order.payment.method]}</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">Estimated delivery</p>
              <p className="mt-1 text-sm text-(--color-text-strong)">{etaStr}</p>
            </div>
            <div className="col-span-2 pt-3 border-t border-(--color-border)">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">Placed on</p>
              <p className="mt-1 text-sm text-(--color-text-strong)">{orderDateStr}</p>
            </div>
          </div>
        </div>
        ...
        {/* Delivery address */}
        <div className="rounded-lg border border-(--color-border) bg-(--color-surface) p-5 mb-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-(--color-text-muted) mb-2">
            Delivery to
          </p>
          <p className="text-sm font-medium text-(--color-text-strong)">{order.address.name}</p>
          <p className="text-sm text-(--color-text) flex items-center gap-1.5 mt-0.5">
            <Phone size={12} className="text-(--color-text-muted)" aria-hidden="true" />
            {order.address.phone}
          </p>
          <p className="text-sm text-(--color-text) flex items-start gap-1.5 mt-0.5">
            <MapPin size={12} className="mt-0.5 text-(--color-text-muted) shrink-0" aria-hidden="true" />
            <span>
        ...
        {/* Tracking */}
        <div className="rounded-lg border border-(--color-border) bg-(--color-surface) p-5 mb-5">
          <h2 className="text-base font-semibold text-(--color-text-strong) mb-5">Order tracking</h2>
          <ol className="relative space-y-5">
            <span
              className="absolute left-[15px] top-2 bottom-2 w-px bg-(--color-border)"
              aria-hidden="true"
            />
            {STEP_DEFS.map((step, i) => {
              ...
              return (
                <li key={step.key} className="flex items-center gap-3 relative">
                  <span
                    className={cn(
                      "w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 border-2 bg-(--color-bg)",
                      done
                  ...
                  <div>
                    <p
                      className={cn(
                        "text-sm font-medium",
                        done ? "text-(--color-text-strong)" : "text-(--color-text-muted)"
                      )}
                    >
                      {step.label}
                    </p>
        ...
        {/* Items */}
        <div className="rounded-lg border border-(--color-border) bg-(--color-surface) p-5 mb-5">
          <h2 className="text-base font-semibold text-(--color-text-strong) mb-4">Items</h2>
            ...
            {order.items.map((it) => (
              <li key={it.id} className="flex gap-3 items-center">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-(--color-text-strong) line-clamp-1">{it.name}</p>
                  <p className="text-xs text-(--color-text-muted)">Qty: {it.quantity}</p>
                </div>
                <span className="text-sm font-semibold tabular text-(--color-text-strong) shrink-0">
