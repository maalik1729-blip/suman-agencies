...
import { getLineTotalINR } from "@/data/products";
  ...
  const handleConfirm = async () => {
    ...
    const order: Order = {
      ...
      items: items.map((i) => ({
        ...
        unitPriceINR: getLineTotalINR(i.product, i.quantity) / i.quantity,
        quantity: i.quantity,
        variant: i.color,
      })),
      address: {
                ...
                {items.map((item) => (
                  <li key={item.product.id} className="flex gap-3 items-center">
                    ...
                    <span className="text-xs font-semibold tabular text-(--color-text-strong) shrink-0">
                      {formatPrice(getLineTotalINR(item.product, item.quantity))}
                    </span>
                  </li>
                ))}
              </ul>
