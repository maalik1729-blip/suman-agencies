...
function TrustStrip() {
  ...
  return (
    <section
      ...
      className="bg-(--color-surface) border-y border-(--color-border)"
    >
      <ul className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-2 lg:grid-cols-4">
        {items.map(({ icon: Icon, label, href }) => (
          <li key={label}>
            <a
              href={href}
              className="flex items-center gap-2 py-4 text-xs sm:text-sm text-(--color-text) hover:text-(--color-text-strong) transition-colors"
            >
              <Icon size={16} className="text-(--color-brand-500) shrink-0" aria-hidden="true" />
...
function CategorySplit() {
  ...
  return (
    <section className="bg-(--color-bg) py-16 sm:py-20 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {tiles.map((t) => (
            <Link
              key={t.title}
              href={t.href}
              className="group relative aspect-5/4 sm:aspect-4/5 overflow-hidden rounded-lg border border-(--color-border) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--color-brand-500) focus-visible:ring-offset-2"
            >
              <Image
                src={t.image}
                alt=""
...
function TrendingCarousel() {
  ...
  return (
    <section
      className="bg-(--color-surface) py-16 sm:py-20 px-4 sm:px-6 overflow-hidden"
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-8">
          ...
          <div className="flex gap-2">
            <button
              onClick={prev}
              className="inline-flex items-center justify-center w-10 h-10 rounded-md border border-(--color-border) bg-(--color-bg) text-(--color-text) hover:border-(--color-border-strong) hover:text-(--color-text-strong) transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--color-brand-500)"
        ...
        <div className="relative">
          ...
          <div className="mt-8 h-px bg-(--color-border) rounded-full overflow-hidden">
            <div
              className="h-full bg-(--color-brand-500) transition-[width] duration-300"
...
function FeaturedProducts() {
  ...
  return (
    <section
      className="bg-(--color-bg) py-16 sm:py-20 px-4 sm:px-6"
...
function WhyUs() {
  ...
  return (
    <section
      className="bg-(--color-surface) py-16 sm:py-20 px-4 sm:px-6"
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {pillars.map(({ icon: Icon, title, description }) => (
            <div
              key={title}
              className="rounded-lg border border-(--color-border) bg-(--color-bg) p-6"
            >
              <span className="inline-flex items-center justify-center w-10 h-10 rounded-md bg-(--color-brand-50) text-(--color-brand-700)">
                <Icon size={18} aria-hidden="true" />
              </span>
              <h3 className="mt-4 text-lg font-semibold text-(--color-text-strong)">
                {title}
              </h3>
              <p className="mt-2 text-sm text-(--color-text) leading-relaxed">
                {description}
              </p>
            </div>
          ))}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {picks.map((t) => (
            <li
              key={t.id}
              className="rounded-lg border border-(--color-border) bg-(--color-surface) p-6 flex flex-col gap-4"
            >
              <div className="flex items-center gap-3">
                <span
                  className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-(--color-brand-100) text-(--color-brand-700) font-semibold"
                  aria-hidden="true"
                >
                  {t.name.charAt(0)}
                </span>
                <div>
                  <p className="text-sm font-medium text-(--color-text-strong)">{t.name}</p>
                  <p className="text-xs text-(--color-text-muted)">{t.role}</p>
                </div>
              </div>
              <div className="flex items-center gap-1 text-(--color-warning-500)">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    size={12}
                    className={i < t.rating ? "fill-current" : "text-(--color-border)"}
                    aria-hidden="true"
                  />
                ))}
              </div>
              <p className="text-sm text-(--color-text) leading-relaxed">"{t.text}"</p>
...
function FinalCTA() {
  return (
    <section
      className="bg-(--color-surface-2) border-y border-(--color-border) px-4 sm:px-6 py-14 sm:py-16"
      aria-labelledby="final-cta"
    >
      <div className="max-w-3xl mx-auto text-center">
        <h2
          id="final-cta"
          className="font-display font-semibold text-2xl sm:text-3xl tracking-tight text-(--color-text-strong)"
        >
          Need help choosing?
        </h2>
        <p className="mt-3 text-sm sm:text-base text-(--color-text) max-w-xl mx-auto">
