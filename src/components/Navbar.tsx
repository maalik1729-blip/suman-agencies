...
export function Navbar() {
  ...
  return (
    <>
      <header
        className="sticky top-0 z-50 bg-(--color-bg) border-b border-(--color-border)"
      ...
      >
        <nav className="max-w-7xl mx-auto h-full px-4 sm:px-6 flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="md:hidden inline-flex items-center justify-center w-9 h-9 rounded-md text-(--color-text-strong) hover:bg-(--color-surface-2) transition-colors"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={toggleTheme}
              className="inline-flex items-center justify-center w-9 h-9 rounded-md text-(--color-text-strong) hover:bg-(--color-surface-2) transition-colors"
      ...
      >
        ...
        <div
          ...
          className={cn(
            "relative h-full w-[88vw] max-w-sm bg-(--color-bg) shadow-xl flex flex-col transition-transform duration-300",
        ...
        >
          <div className="flex items-center justify-between p-4 border-b border-(--color-border)">
            ...
            <button
              onClick={() => setMobileOpen(false)}
              className="inline-flex items-center justify-center w-9 h-9 rounded-md text-(--color-text-strong) hover:bg-(--color-surface-2)"
          ...
          <nav className="flex-1 overflow-y-auto p-4 flex flex-col gap-1">
            ...
            <div className="px-3">
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.08em] text-(--color-text-muted)">
                Currency
              </p>
            ...
            <div className="px-3 mt-4">
              ...
              <button
                onClick={toggleTheme}
                className="inline-flex items-center gap-2 h-9 px-3 rounded-md border border-(--color-border) bg-(--color-surface) text-sm text-(--color-text-strong)"
              >
            ...
            <div className="px-3 text-sm text-(--color-text-muted) space-y-2">
              {site.contact.phones.map((p) => (
                <a key={p.e164} href={`tel:${p.e164}`} className="block hover:text-(--color-text-strong)">
                  📞 {p.display}
                </a>
              ))}
              <p className="text-xs">
