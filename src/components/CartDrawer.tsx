        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3" aria-label="Cart items">
              {items.map((item) => {
                const bulk = isBulkProduct(item.product);
                const step = bulk ? BULK_PACK_SIZE : 1;
                const lineTotal = getLineTotalINR(item.product, item.quantity);
                const qtyLabel = bulk
                  ? `${item.quantity / BULK_PACK_SIZE} ${item.quantity / BULK_PACK_SIZE === 1 ? "pack" : "packs"}`
                  : `${item.quantity}`;
                return (
                <li
                  key={item.product.id}
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <div className="inline-flex items-center rounded-md border border-(--color-border) overflow-hidden">
                        ...
                        <span className="min-w-14 px-1 text-center text-sm font-medium text-(--color-text-strong) tabular">
                          {qtyLabel}
                        </span>
                        <button
                          onClick={() => updateQuantity(item.product.id, item.quantity + step)}
