...
const imagesList = fs.readdirSync('d:/ziya/furniture/public/products').filter(f => f.endsWith('.jpg') && f.startsWith('ata_images_'));

let content = fs.readFileSync('d:/ziya/furniture/src/data/products.ts', 'utf-8');

// Quick fix to clean up any garbage strings from the previous failed run
content = content.replace(/\/products\/[^"]+/g, 'REMOVED');
